...
// Mocks provisórios antes da integração com a API Node.js final do backend
const cashflowData = [
...
export default function DashboardHome() {
  return (
    <motion.div variants={staggerKids} initial="hidden" animate="show" className="max-w-7xl mx-auto space-y-8">
      
      {/* Header */}
      <motion.div variants={childVariant} className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight mb-1">Olá, <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-400">Usuário</span>!</h1>
          <p className="text-slate-400">Aqui está o resumo do seu patrimônio atualizado.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button className="px-5 py-2.5 rounded-xl bg-primary text-primary-foreground font-medium hover:opacity-90 transition-opacity shadow-lg shadow-primary/20">
            Nova Receita
          </button>
        </div>
      </motion.div>
      ...
      {/* Top Cards Stats */}
      <motion.div variants={childVariant} className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Main Chart Flow */}
        <motion.div variants={childVariant} className="lg:col-span-2 glass-panel rounded-2xl p-6 flex flex-col">
          ...
          <div className="h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={cashflowData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                ...
                />
                <Area type="monotone" dataKey="income" name="Receita" stroke="#3b82f6" strokeWidth={3} fillOpacity={1} fill="url(#colorIncome)" />
        ...
        <motion.div variants={childVariant} className="space-y-6 flex flex-col">
          
          {/* AI Advice Widget */}
          <div className="glass-panel rounded-2xl p-6 bg-gradient-to-br from-indigo-900/40 to-slate-900/60 border-indigo-500/20 relative overflow-hidden flex-1 group cursor-pointer hover:border-indigo-500/40 transition-colors">
            <div className="absolute top-0 right-0 p-4 opacity-20">
              <Sparkles className="w-24 h-24 text-indigo-400 blur-[2px]" />
            </div>
            <div className="relative z-10">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/20 text-indigo-300 text-xs font-bold mb-4 border border-indigo-500/30">
                <Sparkles className="w-3 h-3" /> Dica da IA
              </div>
              <h3 className="text-lg font-bold text-slate-100 mb-2 leading-tight group-hover:text-indigo-300 transition-colors">Reduza Gasto com Moradia</h3>
              <p className="text-sm text-slate-400 line-clamp-4">
                Notei que seus gastos em Moradia este semestre estão 22% mais altos que no último. Sugiro revisar faturas não essenciais como streaming antes de fechar o mês.
              </p>
            </div>
          </div>
          ...
          <div className="glass-panel rounded-2xl p-6 flex-1">
             <div className="flex items-center justify-between mb-4">
               ...
               <span className="text-xs text-slate-500 bg-slate-800 px-2 py-0.5 rounded-full">Brapi API</span>
             </div>
